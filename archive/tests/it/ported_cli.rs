//! Tests ported from https://github.com/fossas/fossa-cli, using the experimentation mode instead of extension mode.
...
fn simple_content<'a>() -> Vec<(&'a str, &'a [u8])> {
    vec![
...
fn el7_content<'a>() -> Vec<(&'a str, &'a str)> {
    vec![
        (
            "usr/bin/curl",
...
fn fc35_content<'a>() -> Vec<(&'a str, &'a str)> {
    vec![
        (
            "usr/bin/curl",
