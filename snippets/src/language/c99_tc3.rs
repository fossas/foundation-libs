...
fn extract_function<L>(
    ...
    let text = extract_text(meta.method(), &context, &loc, context_bytes);
...
fn extract_text<'a, 'b>(
    ...
    match method {
        ...
        SnippetMethod::Normalized(tf) => transform(tf, context, location, content),
    }
}
...
fn transform<'a, 'b>(
    ...
    match transform {
        ...
        SnippetTransform::Comment => normalize_comments(context, location, content),
