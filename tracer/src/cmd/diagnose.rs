...
pub fn main(args: Arguments, report_trace_level: Level) -> anyhow::Result<()> {
    ...
    let file = File::open(&args.trace_file_path).context("open trace file")?;
    ...
    for (count, line) in line_reader.enumerate() {
        let line_number = count + 1;
        let line = line.map_err(|err| anyhow!("read line {line_number}: {err:#}"))?;
        if let Entry::Log(entry) = serde_json::from_str::<Entry>(&line).context("parse line")? {
